...
ENDPOINT_CACHE = Path(__file__).parent / ".graphql_endpoint"
STATE_VERSION = 2
# Amazon has moved this endpoint several times. We probe candidates in
# order and cache the first one that responds 2xx. Set GRAPHQL_URL env
# var to pin a specific endpoint (e.g. one grabbed from DevTools).
GRAPHQL_CANDIDATES = [
    "https://hiring.amazon.ca/app-api/career-site/graphql",
    "https://hiring.amazon.ca/api/jobs-graphql",
    "https://hiring.amazon.ca/app/api/jobs-graphql",
    "https://hiring.amazon.ca/api/career-site/graphql",
]
...
def _post_once(url: str, payload: dict, headers: dict) -> dict:
    r = requests.post(url, json=payload, headers=headers, timeout=30)
    if r.status_code in (401, 403):
        raise SystemExit(
            f"Amazon rejected request ({r.status_code}) at {url}. Body: "
            f"{_snippet(r.text)}\n"
            "If this persists, open hiring.amazon.ca in your browser, "
            "DevTools > Network, find a 'graphql' request, copy the "
            "Authorization: Bearer ... header and set it as BEARER_TOKEN."
        )
    ...
    if not r.ok:
        raise requests.HTTPError(
            f"{r.status_code} from {url}: {_snippet(r.text)}", response=r
        )
    try:
        data = r.json()
    except ValueError:
        raise requests.HTTPError(
            f"non-JSON {r.status_code} from {url}: {_snippet(r.text)}",
            response=r,
        )
    if data.get("errors"):
...
def _post_with_retry(payload: dict, headers: dict, *, tries: int = 3) -> dict:
    """POST to the cached or pinned endpoint, with retries.
    Falls back to probing GRAPHQL_CANDIDATES if the current one breaks."""
    last_err: Exception | None = None
    url = _resolve_endpoint()
    for attempt in range(1, tries + 1):
        try:
            return _post_once(url, payload, headers)
        except (requests.RequestException, RuntimeError) as e:
            last_err = e
            print(f"  attempt {attempt} failed ({e})")
            # On 404, the endpoint is gone — re-probe and try again next loop.
            if isinstance(e, requests.HTTPError) and getattr(e.response, "status_code", 0) == 404:
                try:
                    ENDPOINT_CACHE.unlink(missing_ok=True)
                except OSError:
                    pass
                url = _probe_endpoint(payload, headers) or url
            if attempt < tries:
...
def _resolve_endpoint() -> str:
    pinned = os.environ.get("GRAPHQL_URL", "").strip()
    if pinned:
        return pinned
    if ENDPOINT_CACHE.exists():
        ...
        if cached:
            return cached
    return GRAPHQL_CANDIDATES[0]


def _probe_endpoint(sample_payload: dict, headers: dict) -> str | None:
    """Try each candidate; cache and return the first 2xx."""
    print("  probing GraphQL endpoint candidates...")
    for url in GRAPHQL_CANDIDATES:
        try:
            r = requests.post(url, json=sample_payload, headers=headers, timeout=15)
            print(f"    {r.status_code}  {url}")
            if r.ok:
                ENDPOINT_CACHE.write_text(url)
                print(f"  found working endpoint: {url}")
                return url
        except requests.RequestException as e:
            print(f"    ERR  {url}  {e}")
    print("  no candidate endpoint responded 2xx")
    return None
...
def fetch_schedules(job_id: str) -> list[dict]:
    ...
    try:
        data = _post_with_retry(payload, _auth_headers(), tries=2)
    except SystemExit:
        return []
